...
const boardStore = create(
  persist(
    (set) => ({
      buckets: [
        { id: "default1", title: "Career", headerImage: "", icon: "", color: "" },
        { id: "default2", title: "Learning", headerImage: "", icon: "", color: "" },
        { id: "default3", title: "Extracurriculars", headerImage: "", icon: "", color: "" },
      ],
      activities: [],
      tasks: [],
      relationships: [],
      ...
      addBucket: (newBucket) => set((state) => ({ buckets: [...state.buckets, newBucket] })),

      updateBucketTitle: (bucketId, newTitle) => set((state) => ({
        buckets: state.buckets.map((bucket) =>
          bucket.id === bucketId ? { ...bucket, title: newTitle } : bucket
        ),
      })),

      updateBucketColor: (bucketId, newColor) => set((state) => ({
        buckets: state.buckets.map((bucket) =>
          bucket.id === bucketId ? { ...bucket, color: newColor } : bucket
        ),
      })),

      updateBucketHeader: (bucketId, newHeader) => set((state) => ({
        buckets: state.buckets.map((bucket) =>
          bucket.id === bucketId ? { ...bucket, headerImage: newHeader } : bucket
        ),
      })),

      addActivity: (bucketId, newActivity) => set((state) => ({
        activities: [...state.activities, newActivity],
      })),

      editActivity: (activityId, updatedActivity) => set((state) => ({
      ...
      deleteActivity: (activityId) => set((state) => {
        ...
        const updatedTasks = state.tasks.filter((task) => task.activityId !== activityId);
        const updatedRelationships = state.relationships.filter(
          (rel) => rel.activity1Id !== activityId && rel.activity2Id !== activityId
        );
      ...
      addRelationship: (newRelationship) => set((state) => ({
      ...
      setPriority: (activityId, priority) => set((state) => ({
        activities: state.activities.map((activity) =>
      ...
      addTimeBlock: (activityId, timeblockObject) => set((state) => ({
        timeblocks: [...state.timeblocks, { ...timeblockObject, activityId }]
      })),
      updateTimeBlock: (activityId, timeblockObject) => set((state) => {
